# Коллекция данных. Множества. Задание "Супермена"
	...
	for id1 in range(len(mentors_names)):
		for id2 in range(len(mentors_names)):
			if id1 != id2:
				...
				if pairs.count(intersection_set) == 0:
					pairs.append(intersection_set)
					result_prom = sorted(list(intersection_set))
					result.append(result_prom)
					print(f"На курсах '{courses[id1]}' и '{courses[id2]}' преподают: {', '.join(result_prom)}")
	return result

res = prepods_in_courses()